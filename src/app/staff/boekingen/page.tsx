    ...
    <div className="bg-surface rounded-2xl p-5 mt-2 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <div className="flex items-center gap-1.5 text-xs text-muted mt-1">
              ...
              <a href={`mailto:${booking.guest_email}`} className="hover:text-emerald-600">{booking.guest_email}</a>
            </div>
            <div className="flex items-center gap-1.5 text-xs text-muted mt-0.5">
              <Phone className="w-3 h-3" />
              <a href={`tel:${booking.guest_phone}`} className="hover:text-emerald-600">{booking.guest_phone}</a>
            </div>
          </div>
          <div>
            ...
            {payments.map((p) => (
              <div key={p.id} className="bg-white rounded-xl p-3 flex items-center justify-between text-sm">
                <div>
                  ...
                  {p.paid_at && <p className="text-xs text-emerald-600">Betaald op {formatDateTime(p.paid_at)}</p>}
                </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 bg-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
          >
        ...
        <div>
          <label className="text-xs font-semibold text-muted uppercase tracking-wider block mb-1">Notities</label>
          <textarea
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            rows={2}
            className="w-full px-3 py-2 bg-white rounded-xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-emerald-500"
      ...
      {(newStatus !== booking.status || notes !== (booking.admin_notes || '')) && (
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm font-medium hover:bg-emerald-700 transition-colors cursor-pointer disabled:opacity-50"
        >
...
export default function StaffBookingenPage() {
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
          />
        </div>
        <div className="relative">
          ...
          <select
            ...
            className="pl-10 pr-8 py-2.5 bg-white rounded-xl text-sm appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500"
          >
