...
export default function PrijzenPage() {
  ...
  const openCreate = useCallback((preset?: typeof PRESETS_NL[0]) => {
    setEditId(null);
    if (preset) {
  ...
  usePageActions(
    useMemo(() => (
      <>
        <button onClick={() => fetchRules()} className="p-2 bg-white rounded-xl text-muted hover:text-primary transition-colors cursor-pointer" title={isNL ? 'Vernieuwen' : 'Refresh'}>
          <RefreshCw size={14} />
        </button>
        <button onClick={() => openCreate()} className="p-2 bg-primary-dark text-white rounded-xl hover:bg-primary-dark/90 transition-colors cursor-pointer" title={isNL ? 'Nieuwe regel' : 'New rule'}>
          <Plus size={14} />
        </button>
      </>
    ), [fetchRules, openCreate, isNL])
  );
